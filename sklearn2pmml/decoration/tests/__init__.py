...
class CategoricalDomainTest(TestCase):
	...
	def test_fit_int_missing(self):
		...
		Xt = domain.fit_transform(X)
		self.assertIsInstance(Xt, DataFrame)
		self.assertEqual(2, len(domain.data_))
		self.assertEqual([0, 1, 2, 3], domain.data_[0].tolist())
		self.assertEqual([0, 1, 2], domain.data_[1].tolist())
	...
	def test_fit_string_missing(self):
		...
		self.assertFalse(domain._empty_fit())
		X = DataFrame([["1", "NA"], ["NA", "one"], ["3", "two"], ["2", "N/A"], ["N/A", "NA"], ["2", "three"]])
		Xt = domain.fit_transform(X)
		self.assertIsInstance(Xt, DataFrame)
		self.assertEqual(2, len(domain.data_))
		self.assertEqual(["0", "1", "2", "3"], domain.data_[0].tolist())
		self.assertEqual(["0", "one", "three", "two"], domain.data_[1].tolist())
		self.assertEqual(2, len(domain.counts_))
		self.assertEqual({"totalFreq" : 6, "missingFreq" : 2, "invalidFreq" : 0}, domain.counts_[0])
		self.assertEqual({"totalFreq" : 6, "missingFreq" : 3, "invalidFreq" : 0}, domain.counts_[1])
		self.assertEqual(2, len(domain.discr_stats_))
		...
		self.assertEqual((6, 2), Xt.shape)
		self.assertEqual(["1", "0", "3", "2", "0", "2"], Xt.iloc[:, 0].tolist())
		self.assertEqual(["0", "one", "two", "0", "0", "three"], Xt[1].tolist())
